...
## smart_report.sh — Monthly Trend Report

### What does this script do?

This companion script reads all per-drive CSV files generated by `smart_check.sh` and sends a monthly email report showing the last 4 measurements per attribute — giving you a clear picture of how each value has changed over the past month.

Example output in the email:
```
--- ata-Samsung_SSD_850_EVO_500GB_S21JNXAG123456 ---
  Wear_Leveling_Count: 098   (Past trend: 100 -> 099 -> 099 -> 098)
  Reallocated_Sector_Ct: 0   (Past trend: 0 -> 0 -> 0 -> 0)

--- nvme-Micron_3400_MTFDKBA512TFH_ABC123 ---
  Error_Log_Entries: 193   (Past trend: 192 -> 192 -> 193 -> 193)
  Percentage_Used: 3        (Past trend: 3 -> 3 -> 3 -> 3)
```
...
**2. Make it executable**
```bash
chmod +x /root/smart_report.sh
```

**3. Add to crontab** (runs monthly, on the 1st at 03:00)
```bash
crontab -e
```
Add this line:
```
0 3 1 * * /root/smart_report.sh
```

> **Note:** Run `smart_check.sh` for at least a few weeks before using this report — it needs historical CSV data to show meaningful trends.
...
## smart_report.sh — Havi Trendriport

### Mit csinál ez a script?

Ez a kísérő script beolvassa a `smart_check.sh` által generált meghajtónkénti CSV fájlokat, és havi e-mail riportot küld, amely attribútumonként az utolsó 4 mérést mutatja — így egyértelműen látható, hogyan változtak az értékek az elmúlt hónapban.

Példa az e-mail tartalmára:
```
--- ata-Samsung_SSD_850_EVO_500GB_S21JNXAG123456 ---
  Wear_Leveling_Count: 098   (Múltbeli trend: 100 -> 099 -> 099 -> 098)
  Reallocated_Sector_Ct: 0   (Múltbeli trend: 0 -> 0 -> 0 -> 0)

--- nvme-Micron_3400_MTFDKBA512TFH_ABC123 ---
  Error_Log_Entries: 193   (Múltbeli trend: 192 -> 192 -> 193 -> 193)
  Percentage_Used: 3        (Múltbeli trend: 3 -> 3 -> 3 -> 3)
```
...
**3. Hozzáadás a crontab-hoz** (havonta egyszer, minden hó 1-jén 03:00-kor fut)
```bash
crontab -e
```
Add hozzá ezt a sort:
...
> **Megjegyzés:** Legalább néhány hétig futtasd a `smart_check.sh`-t mielőtt ezt a riportot használod — értelmes trendhez historikus CSV adatok szükségesek.
